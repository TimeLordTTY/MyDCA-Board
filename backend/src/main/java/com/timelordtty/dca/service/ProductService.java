...
public class ProductService {
        ...
        new Thread(() -> {
            try {
                ...
                if ("EXCHANGE".equals(channel)) {
                    if (isBondRepo(product)) {
                        logger.info("采集债券/国债逆回购实时行情...");
                        String realtimeResult = pythonScriptService.collectBondRealtime();
                        logger.info("债券/国债逆回购实时行情采集完成: {}", realtimeResult);

                        logger.info("采集债券/国债逆回购日线行情...");
                        String dailyResult = pythonScriptService.collectBondDaily();
                        logger.info("债券/国债逆回购日线行情采集完成: {}", dailyResult);
                    } else {
                        // 其他场内产品：采集实时行情和日K线
    ...
    /**
     * 判断产品是否为国债逆回购，用于把债券行情采集与 ETF/股票行情采集分流。
     */
    private boolean isBondRepo(ProductMaster product) {
        return product != null && "BOND_REPO".equals(product.getAssetType());
    }
    ...
    public void refreshAllMarketData() {
        new Thread(() -> {
            try {
                ...
                logger.info("采集债券/国债逆回购实时行情...");
                String bondRealtimeResult = pythonScriptService.collectBondRealtime();
                logger.info("债券/国债逆回购实时行情采集完成: {}", bondRealtimeResult);
                ...
                logger.info("采集债券/国债逆回购日线行情...");
                String bondDailyResult = pythonScriptService.collectBondDaily();
                logger.info("债券/国债逆回购日线行情采集完成: {}", bondDailyResult);
