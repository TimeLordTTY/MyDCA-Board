...
class ProfitRecycleStrategy(Strategy):
    ...
    def on_start(self) -> None:
        ...
        state = self.state
        # 资金统计
        state["principal_total"] = 0.0          # 真实打入账户的总本金（= 所有现金流入之和）
    ...
    def on_bar(self, ctx: Context) -> Signal:
        ...
        # 1. 记录真实打入本金（包括初始 + 每月定投）
        if cash_inflow > 0:
            state["principal_total"] += cash_inflow

        # 2. 处理预投入本金：从当日现金流中抽取一部分记入锁定池
        reserve: float = 0.0
        ...
        # 4. 深跌补仓：只在满足阈值、且本轮尚未触发过的情况下，释放预投入锁定池的一部分
        deep_dip_info = ""
        if (
            last_peak_nav > 0.0
        ...
        # 6. 基线行为：所有未锁定的现金，全部用于买入（模拟纯定投效果）
    ...
    def render_summary(self, summary: Dict[str, Any]) -> None:
        ...
        total_cost: float = float(summary.get("total_cost", 0.0))  # 实际买入成本（下场资金）
        final_value: float = float(summary.get("final_value", 0.0))  # 期末基金市值
        final_cash: float = float(summary.get("final_cash", 0.0))    # 期末账户现金

        # 名义收益（只看下场资金）
        nominal_pl = final_value + final_cash - total_cost
        nominal_return = nominal_pl / total_cost if total_cost > 0 else 0.0

        # 校准视角：使用真实打入本金（principal_total）
        principal_total: float = float(self.state.get("principal_total", 0.0))
        real_final_assets = final_value + final_cash
        real_pl = real_final_assets - principal_total
        real_return = real_pl / principal_total if principal_total > 0 else 0.0

        # 预投入体系的统计
        pre_invest_locked: float = float(self.state.get("pre_invest_locked", 0.0))
        pre_invest_total_in: float = float(self.state.get("pre_invest_total_in", 0.0))
        pre_invest_released: float = float(self.state.get("pre_invest_released", 0.0))
        deep_dip_count: int = int(self.state.get("deep_dip_count", 0))
        last_peak_nav: float = float(self.state.get("last_peak_nav", 0.0))

        # 结束时，预投入体系中“仍锁定”的那部分现金，理论上不应超过 final_cash
        # 但如果策略在最后几天刚刚锁了一笔钱，可能会出现 locked > final_cash 的极端情况
        # 此处仅用于展示，不做强行纠正
        estimated_locked_cash_in_final = min(pre_invest_locked, final_cash)
        ...
        print(f"   期末基金市值:      {final_value:12,.2f} 元")
        print(f"   期末现金余额:      {final_cash:12,.2f} 元")
        print(f"   期末总资产:        {real_final_assets:12,.2f} 元\n")

        print("【收益情况】")
        print(f"   基于 total_cost 的名义盈亏金额: {nominal_pl:12,.2f} 元")
        print(f"   基于 total_cost 的名义总收益率: {nominal_return:8.2%}")
        print(f"   年化收益率（名义）:            {summary.get('cagr', nominal_return):8.2%}\n")
        ...
        print(f"   预估期末仍锁定的现金(<=final_cash):      {estimated_locked_cash_in_final:12,.2f} 元")
        print(f"   深跌补仓触发次数 (deep_dip_count):       {deep_dip_count:12d}")
        print(f"   观测最高净值 (last_peak_nav):              {last_peak_nav:.4f}\n")

        print("【校准视角】")
        print(f"   实际买入成本 total_cost:           {total_cost:12,.2f} 元")
        print(f"   真实打入本金 principal_total:      {principal_total:12,.2f} 元")
        print(f"   期末总资产(基金市值+现金):         {real_final_assets:12,.2f} 元")
        print(f"   ▶ 名义收益率(对下场资金):              {nominal_return:8.2%}")
        print(f"   ▶ 真实收益率(对全部本金):              {real_return:8.2%}\n")

        print("=" * 70)
        print(f"✅ 回测完成（{self.get_name()}.render_summary）")
        print("=" * 70 + "\n")
