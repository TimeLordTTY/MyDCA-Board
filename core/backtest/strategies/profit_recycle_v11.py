...
class ProfitRecycleStrategy(Strategy):
    ...
    def on_bar(self, ctx: Context) -> Signal:
        nav = float(ctx.nav)
        dt_val = ctx.dt  # datetime.date / datetime.datetime
        cash_inflow = float(ctx.cash_inflow or 0.0)
        cash_pool = float(ctx.cash_pool or 0.0)
        units = float(getattr(ctx, "units", 0.0) or ctx.portfolio.get("units", 0.0) or 0.0)
        ...
        # === 把本次现金流入的一部分“计入锁定池”（注意：引擎里现金仍在 cash_pool，本策略用 pre_locked 记账来限制当期可买金额）===
        if cash_inflow > 0:
        ...
        if deep_dip_triggered and self.allow_multi_deep_dip:
            ...
            if last_dip_date is not None:
                try:
                    days = (dt_val - last_dip_date).days
        ...
        if not deep_dip_triggered:
            ...
            if matched is not None and pre_locked > 1e-6:
                ...
                state[self.K_LAST_DIP_DATE] = dt_val
        ...
        if self.take_profit_enabled and units > 1e-12 and ma is not None:
            ...
            if tp_ok:
                ...
                if last_tp_date is not None:
                    try:
                        if (dt_val - last_tp_date).days < self.take_profit_cooldown_days:
                            tp_ok = False
                    except Exception:
                        tp_ok = False
            ...
            if tp_ok:
                ...
                if sell_units > 1e-12:
                    ...
                    state[self.K_LAST_TP_DATE] = dt_val
        ...
        # === 可用现金：cash_pool - locked（锁定池越大，可买越少）===
        # 注意：由于引擎里现金就是 cash_pool，本策略用“locked 记账”限制买入现金。
        locked_cash = max(0.0, min(pre_locked, cash_pool))
        ...
        if sell_units <= 1e-12:
            ...
            buy_cash = max(0.0, min(buy_cash, cash_pool))
