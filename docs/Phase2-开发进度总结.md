...
- ✅ **今日建议清单**（当前已接入“待结算/逾期待处理”动作，完整策略建议顺延至后续策略阶段）
...
说明：本节为 Phase2 Mobile H5 历史交付记录。Phase3 起移动端主线由 Android 原生 App 替代，`web/mobile-app` 可保留作为历史参考或兼容入口，不再作为主移动端交付形态；新增移动端能力、自动记账、草稿箱和长期体验优化统一迁移至 Phase3 原生 App 实现。
...
- [ ] BOLL / KDJ 等扩展指标
- [ ] 持仓成本分析 / 持仓收益统计
- [ ] 完整策略建议闭环（顺延至 Phase 4 策略阶段）
- [ ] 复盘导出与回测实验室（Phase 4/5）
...
#### 6. 指标计算扩展（计划放入 Phase 4“策略阶段”）
- [ ] 布林带（BOLL）指标
- [ ] KDJ指标
- [ ] 其他常用技术指标（CCI、DMI、OBV等）

#### 7. 前端功能增强（移动端增强迁移至 Phase3 原生 App，策略/导出类能力顺延至 Phase4+）
- [ ] 行情数据缓存机制
- [ ] 实时行情轮询更新（当前为手动刷新）
