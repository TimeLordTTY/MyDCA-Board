...
**阶段状态**：进行中。Phase 2 的双端前端、行情、指标、定时任务高优先级闭环已完成；流水统计功能已落地，BOLL/KDJ 扩展指标首版已完成，债券/国债逆回购行情采集首版已完成。
...
- ✅ **今日建议清单**（当前已接入“待结算/逾期待处理”动作，完整策略建议顺延至 Phase4 策略建议闭环）
...
说明：本节为 Phase2 Mobile H5 历史交付记录。Phase3 起移动端长期形态由 Android 原生 App 替代，但 Phase3 第一交付物调整为“服务端草稿 + Hermes 对话式记账 MVP + 影响预览 + 确认入账”；Android 原生 App 作为 Phase3 后半段体验承接。`web/mobile-app` 可保留作为历史参考或兼容入口，不再作为主移动端交付形态。
...
#### 中优先级（可选优化）
- [ ] **响应式设计优化**：完善移动端适配，优化小屏幕显示（涉及 Mobile 端的长期优化迁移至 Phase3 原生 App 统一实现；Mobile H5 仅保留历史兼容）
...
- `txnTypes`：交易类型多选，如 `EXPENSE,INCOME,REIMBURSE_IN`。
...
- [ ] 建议补齐 `categories` 主数据表或后端分类字典接口，避免统计接口只能返回 `categoryId`。
- [ ] 前端现有 `web/shared/src/constants/categories.ts` 可作为初始分类数据来源，后续与数据库分类表对齐。
...
└── bond_collector.py     # 债券/国债逆回购行情采集首版 ✅
```
...
- ✅ **定时数据采集**：
  ...
  - ✅ 实时行情数据采集任务（Python APScheduler 为交易时间内每5分钟；Java `MarketDataScheduler` 为交易时间内每分钟并带冷却）
  - ✅ 基金净值采集任务（18:00）
- ✅ **定时指标计算**：
  - ✅ 每日指标计算任务（16:00）
  - ✅ 指标数据更新任务
...
**定时任务配置**：
- 基金净值采集：每天 18:00
- ETF实时行情：Python APScheduler 为交易时间内每5分钟；Java `MarketDataScheduler` 为交易时间内每分钟（9:30-11:30、13:00-15:00，带 90 秒冷却）
- ETF日K线：每天 15:30（收盘后）
...
- [x] 债券/国债逆回购行情采集首版（当前覆盖 `BOND_REPO` 场内产品）
...
- [x] 债券/国债逆回购行情采集首版（当前覆盖 `BOND_REPO`，普通债券/可转债待后续扩展）
...
#### Mobile端 ✅
- 说明：以下验收项为 Phase2 Mobile H5 历史交付验收；Phase3 起移动端主线切换为原生 App。
- ✅ 项目基础结构已创建
