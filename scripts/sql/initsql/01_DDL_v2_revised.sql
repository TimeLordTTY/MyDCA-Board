...
CREATE TABLE `ledger_txn` (
  ...
  `relation_type` ENUM('NONE','TRANSFER_PAIR','REFUND','REIMBURSE','REVERSAL') NOT NULL DEFAULT 'NONE' COMMENT '关联类型（NONE=无关联，TRANSFER_PAIR=转账成对，REFUND=退款，REIMBURSE=报销，REVERSAL=撤销）',
...
-- 3.2 ledger_posting - 分录表（修订：明确借贷方向规则，amount永远为正数）
