...
模拟时序：
1. 12/17 晚：统计一次（有 nav，无扣款）
2. 12/18 早：新 nav 出来，统计一次
3. 12/18 晚：发生 buy_debit（扣款），统计一次（cash>0，shares 不变）
4. 12/19 早：新 nav 出来，统计一次（pnl_day 只由净值变化贡献）
5. 12/19 上午：发生 buy_confirm（份额到账），统计一次（shares/cost 更新，cash 归零，但 pnl_day 不跳变）

验证要点：
- 同日多次统计只保留一条（覆盖）
- 扣款后 cash 增加、shares 不变
- 份额到账后 shares 增加、cash 减少
- pnl_day 在"扣款/到账"前后不因为份额变化而跳变
"""
import sys
import os
import csv
...
def setup_test_data(test_dir: Path):
    ...
    with open(products_json, 'w', encoding='utf-8') as f:
        f.write('''[
    {"product_code": "TEST001", "product_name": "测试基金A", "category": "fund"}
]''')
...
def create_snapshot_with_date(nav_records, holdings_map, products_map, snapshot_path, 
    ...
    """创建指定日期的快照（通过 mock datetime.now）"""
...
def run_scenario(test_dir: Path):
    ...
    # ============ 场景 1: 12/17 晚 - 初始状态 ============
    print("\n" + "="*60)
    print("场景 1: 12/17 晚 - 初始状态（有 nav，无扣款）")
    print("="*60)
    ...
    # ============ 场景 2: 12/18 早 - 新 NAV ============
    print("\n" + "="*60)
    print("场景 2: 12/18 早 - 新 nav 出来（1.5000 → 1.5200）")
    print("="*60)
    ...
    # 预期 pnl_day = 1000 * (1.52 - 1.50) = 20
    print(f"  shares={row['shares']}, cash={row['cash']}, value={row['value']}, pnl_day={row['pnl_day']}")
    print(f"  ✓ 预期 pnl_day = 1000 * (1.52 - 1.50) = 20.00")
    
    # ============ 场景 3: 12/18 晚 - 发生扣款 ============
    print("\n" + "="*60)
    print("场景 3: 12/18 晚 - 发生 buy_debit（扣款 100 元）")
    print("="*60)
    
    # 添加扣款记录
    with open(transactions_path, 'a', encoding='utf-8') as f:
        f.write("2025-12-18,TEST001,buy_debit,100,,,,,ORD001,扣款测试\n")
    ...
    print(f"  ✓ 预期: shares 不变(1000), cash 增加(100), pnl_day 不跳变(20)")
    
    # ============ 场景 4: 12/19 早 - 新 NAV（价格变化）============
    print("\n" + "="*60)
    print("场景 4: 12/19 早 - 新 nav 出来（1.5200 → 1.5500）")
    print("="*60)
    
    nav_records = {'TEST001': {'nav': '1.5500', 'nav_date': '2025-12-19'}}
    row = create_snapshot_with_date(
        nav_records,
        {'TEST001': 1000},
        {'TEST001': '测试基金A'},
    ...
    # 预期 pnl_day = 1000 * (1.55 - 1.52) = 30（按上一日已确认份额计算）
    print(f"  shares={row['shares']}, cash={row['cash']}, value={row['value']}, pnl_day={row['pnl_day']}")
    print(f"  ✓ 预期 pnl_day = 1000 * (1.55 - 1.52) = 30.00（扣款不影响 pnl_day）")
    
    # ============ 场景 5: 12/19 上午 - 份额确认 ============
    print("\n" + "="*60)
    print("场景 5: 12/19 上午 - buy_confirm（确认 65 份）")
    print("="*60)
    ...
    print(f"  shares={row['shares']}, cash={row['cash']}, value={row['value']}, total_value={row['total_value']}, pnl_day={row['pnl_day']}")
    print(f"  ✓ 预期: shares 增加(1065), cash 归零(0), pnl_day 仍按上一日份额计算(30)")
    ...
    print("验证结果汇总")
    print("="*60)
    print(f"{'场景':<20} {'shares':>10} {'cash':>10} {'value':>12} {'total_value':>12} {'pnl_day':>10}")
    print("-" * 76)
    for r in results:
        print(f"{r['scenario']:<20} {r['shares']:>10} {r['cash']:>10} {r['value']:>12} {r['total_value']:>12} {r['pnl_day']:>10}")
...
def main():
    ...
    try:
        ...
        # 断言 1: 扣款后 cash 应该增加
        r3 = results[2]  # 场景 3
        if Decimal(r3['cash']) > 0:
            print("✓ 断言1: 扣款后 cash > 0")
        else:
            print("✗ 断言1: 扣款后 cash 应该 > 0")
            all_passed = False
        
        # 断言 2: 扣款后 shares 不变
        r2 = results[1]  # 场景 2
        if r2['shares'] == r3['shares']:
            print("✓ 断言2: 扣款后 shares 不变")
        else:
            print(f"✗ 断言2: 扣款后 shares 应该不变 ({r2['shares']} vs {r3['shares']})")
            all_passed = False
        
        # 断言 3: 确认后 shares 增加
        r5 = results[4]  # 场景 5
        if Decimal(r5['shares']) > Decimal(r3['shares']):
            print("✓ 断言3: 确认后 shares 增加")
        else:
            print(f"✗ 断言3: 确认后 shares 应该增加 ({r3['shares']} -> {r5['shares']})")
            all_passed = False
        
        # 断言 4: 确认后 cash 归零
        if Decimal(r5['cash']) == 0:
            print("✓ 断言4: 确认后 cash 归零")
        else:
            print(f"✗ 断言4: 确认后 cash 应该归零（实际: {r5['cash']}）")
            all_passed = False
        
        # 断言 5: pnl_day 场景2应该是20
        r2_pnl = Decimal(r2['pnl_day'])
        if abs(r2_pnl - Decimal('20.00')) < Decimal('0.01'):
            print("✓ 断言5: 场景2 pnl_day = 20.00")
        else:
            print(f"✗ 断言5: 场景2 pnl_day 应该为 20.00（实际: {r2_pnl}）")
            all_passed = False
        
        # 断言 6: pnl_day 场景4应该是30
        r4 = results[3]  # 场景 4
        r4_pnl = Decimal(r4['pnl_day'])
        if abs(r4_pnl - Decimal('30.00')) < Decimal('0.01'):
            print("✓ 断言6: 场景4 pnl_day = 30.00（扣款不影响）")
        else:
            print(f"✗ 断言6: 场景4 pnl_day 应该为 30.00（实际: {r4_pnl}）")
            all_passed = False
        
        # 断言 7: pnl_day 场景5应该仍然是30（确认不影响当日pnl_day）
        r5_pnl = Decimal(r5['pnl_day'])
        if abs(r5_pnl - Decimal('30.00')) < Decimal('0.01'):
            print("✓ 断言7: 场景5 pnl_day = 30.00（确认不影响当日pnl_day）")
        else:
            print(f"✗ 断言7: 场景5 pnl_day 应该为 30.00（实际: {r5_pnl}）")
            all_passed = False
