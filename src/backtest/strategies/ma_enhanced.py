...
@register_strategy("ma_enhanced", version="v2", set_as_default=True)
class MovingAverageEnhancedStrategy(Strategy):
    ...
    2. 真正实现"高估存钱、低估用钱"：
        - 定投日（有 cash_inflow）：
            * 按估值因子调整当月 500 的实际投入；
            * 多退少补：高估少买，差额留在现金账户作为"子弹"；
        - 非定投日（无 cash_inflow）：
            * 只有当 factor_raw > 1.0（明显低估）时，才允许动用现金抄底；
            * 否则不买，让钱继续躺着等更好的机会；
    3. 统计口径更严谨：
    ...
    def on_bar(self, ctx: Context) -> Signal:
        ...
        # 2. 读取资金信息
        # -----------------------------
        cash: float = float(ctx.cash)  # 使用统一的 cash 字段
        ...
        if not self.state.get("initialized", False):
            ...
            if self.first_day_full_invest and cash > 0:
                # 首日全仓买入
                buy_cash = cash
