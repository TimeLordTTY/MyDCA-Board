...
class StrategyRegistry:
    ...
    def get_strategy_info(self, strategy_name: str, version: Optional[str] = None) -> Dict[str, Any]:
        """
        获取策略信息（包括默认参数和 schema）
        ...
        """
        try:
            strategy_class = self.get(strategy_name, version)
            instance = strategy_class()
            
            if instance is None:
                raise ValueError(f"策略类 {strategy_name} 创建实例失败")
            
            return {
                'strategy_key': instance.get_strategy_key(),
                'strategy_version': instance.get_version(),
            ...
            return {
                ...
                'strategy_version': version or 'default',
...
def get_strategy_info(strategy_name: str, version: Optional[str] = None) -> Dict[str, Any]:
    """获取策略信息（便捷函数）"""
    return _registry.get_strategy_info(strategy_name, version)
